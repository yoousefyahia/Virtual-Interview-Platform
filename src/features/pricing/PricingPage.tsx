  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      <section className="mt-14 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {plans.map((plan) => {
          ...
          return (
            <div
              key={plan.name}
              className={`relative rounded-3xl hover:shadow-hover aspect-auto max-w-100 min-w-60 m-auto text-black bg-white p-8  ${
                isPremium ? "border-4 border-primary " : ""
              }`}>
              <div className="flex items-center justify-between gap-3">
                <div>
                  <p className={`sm:text-lg font-semibold `}>{plan.name}</p>
                  <div className="mt-6 flex items-end gap-2">
                    <span className={`text-3xl sm:text-5xl font-bold `}>{plan.price}</span>
                    <span className={`pb-1 text-xs sm:text-sm `}>{plan.frequency}</span>
                  </div>
                </div>
                {plan.badge ? (
                  <span className="rounded-full bg-primary px-3 py-1 text-center text-xs sm:text-base font-semibold absolute top-0 left-1/2  -translate-1/2 uppercase text-white">
                    {plan.badge}
                  </span>
                ) : null}
              </div>

              <ul className={`mt-8 space-y-1  sm:space-y-3 text-xs sm:text-sm leading-5 sm:leading-6 `}>
              ...
              <Link
                href={`/pricing/confirm?plan=${plan.name.toLowerCase()}`}>
                  
                <button
                  type="button"
                  className={`mt-5 sm:mt-10 inline-flex w-full  max-w-80 m-auto items-center justify-center rounded-2xl px-5 py-3 sm:text-lg font-semibold transition focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 ${
      ...
      <section className="mt-16 overflow-hidden">
        <div className="grid gap-px text-center uppercase grid-cols-4 px-6 py-4 font-bold text-[10px] sm:text-xl">
          <h6>Compare Plans</h6>
          <h6>Basic</h6>
          <h6>Premium</h6>
          <h6>Enterprise</h6>
        ...
        <div className="divide-y gap-y-8">
          {compareRows.map((row) => (
            <div
              ...
              <div className="text-center text-xs sm:text-lg wrap-break-word font-medium  w-1/4">
                {row.label}
              </div>
              {row.values.map((value, index) => (
                <div
                  ...
                  {value ? (
                    <Check className="h-4 w-4 p-px sm:h-7 sm:w-7 sm:p-1  rounded-full bg-primary text-white" />
                  ) : (
                    <span className="inline-flex items-center justify-center text-neutral-800 text-sm sm:text-2xl font-extrabold">
                      —
                    </span>
                  )}
                </div>
      ...
      <section className="mt-16">
        <h2 className=" sm:text-2xl font-semibold text-slate-950">
        ...
        <div className="mt-6 space-y-4">
          {faqs.map((faq) => (
            <details
              ...
              <summary className="flex cursor-pointer items-center justify-between gap-4 list-none text-sm sm:text-base font-medium text-slate-900">
                {faq.question}
                <ChevronDown />
              </summary>
              <p className="mt-4 text-xs sm:text-sm sm:leading-7 leading-5  text-slate-600">
                {faq.answer}
              </p>
            </details>
          ))}
